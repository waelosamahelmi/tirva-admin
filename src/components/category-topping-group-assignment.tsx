...
import {
  ...
  useAssignToppingGroupToCategory,
  useRemoveToppingGroupFromCategory,
...
interface CategoryToppingGroupAssignmentProps {
  categoryId: number;
  categoryName: string;
}

export function CategoryToppingGroupAssignment({ categoryId, categoryName }: CategoryToppingGroupAssignmentProps) {
  const { t } = useLanguage();
  const { toast } = useToast();
  const { data: allToppingGroups } = useToppingGroups();
  const { data: assignedGroups, refetch } = useCategoryToppingGroups(categoryId);
  const assignGroup = useAssignToppingGroupToCategory();
  const removeGroup = useRemoveToppingGroupFromCategory();
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: t("Täydennysryhmä liitetty kategoriaan", "Topping group assigned to category"),
      });
  ...
  const handleRemove = async (toppingGroupId: number) => {
    try {
      ...
      toast({
        title: t("Onnistui", "Success"),
        description: t("Täydennysryhmä poistettu kategoriasta", "Topping group removed from category"),
      });
    ...
    <Card className="mt-4">
      <CardHeader>
        <CardTitle className="text-lg flex items-center gap-2">
          ...
          {t("Täydennysryhmät kategorialle:", "Topping Groups for:")} {categoryName}
        </CardTitle>
      </CardHeader>
          ...
          {assignedGroups && assignedGroups.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {assignedGroups.map((item: any) => {
                ...
                return (
                  <Badge key={item.id} variant="secondary" className="text-sm px-3 py-2">
                    {group.name}
                    <button
            ...
            <p className="text-sm text-gray-500">
              {t("Ei vielä liitettyjä ryhmiä", "No groups assigned yet")}
            </p>
          )}
        </div>
